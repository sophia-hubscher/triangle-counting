...
Triangles, are formed by three mutually connected nodes, as shown in Figure~\ref{fig:triangles}, which shows a graph containing three triangles.
...
Some deterministic methods outlined in more detail in Section \ref{sec:background} decrease the time it takes to compute exact triangle counts \cite{strassen_gaussian_1969}.
...
This thesis aims to extend our understanding of randomized algorithms to find new, fast, accurate ways to estimate triangle counts that can be used in real-world applications.
...
\paragraph{EigenTriangle Algorithm}
...
\paragraph{TraceTriangle Algorithm}
...
In our case, we can begin by finding a relationship between the degree of nodes and the number of triangles they are involved in.
...
Now, using this equation, we can estimate the overall triangle count $\Delta$ by applying this line of best fit relationship to all nodes in the graph:

\[
\Delta \approx M = \sum_{i = 1}^{n} m_i.
\]
...
The implemented methods were applied to both synthetic and real-world networks sourced from the Stanford Network Analysis Platform (SNAP) library \cite{leskovec_snap_2017}.
...
As detailed in Section \ref{sec:importance-sampling-background}, for importance sampling, we attempt to prioritize the sampling of nodes that are likely to form more triangles.
This is done by sampling proportionally to the degree of nodes.
This method aims for higher accuracy with fewer samples.

After we select a value $\alpha$ to indicate the strength of the relationship between the degree and triangle count of nodes, we can use this value $\alpha$ to ascribe each node a probability $p_i$ to each node based in its degree $d_i$:
...
To select the ideal value for the power $\alpha$, as described in Section \ref{sec:importance-sampling-background}, we plot nodes' log degrees verses log triangle counts and set $\alpha$ to be the line of best fit of this plot.
...
In \cref{tab:percent_error_100,tab:percent_error_4000}, we see that for all datasets tested on except for the collaboration network dataset, the hybrid method tended to achieve the lowest percent error at both small and large sample sizes (specially, the table displays data for sample sizes of 100 and 4000).
...
In figures \cref{fig:percent_error_importance,fig:percent_error_variance_importance}, we also find that powers closest to the optimal value $\alpha$, as described in Section \ref{sec:importance-sampling-background} have lowest percent error.
...
These comparisons are not very robust, but they do provide evidence that the patterns observed in the triangle counting experiments are likely generalizable to other motifs.
...
Specifically as described in Section \ref{sec:methods-simulated-test}, we tested variance reduction and importance sampling under two different triangle-degree relationships: uniform noise and multiplicative noise.
...
To check this, we plot true triangle counts vs. `noise' (i.e. the absolute value of the true minus approximate triangle counts) for each dataset.
...
In this thesis, we found a trade-off between power and runtime, finding that powers closest to the optimum for achieving low error did not necessarily achieve the best balance between accuracy and runtime.
Thus, another further research direction would be investigating, and perhaps quantifying, that tradeoff.

Lastly, in this thesis, we extended our analysis out to other motifs, specifically, 4-cliques.
There are many more graph motifs out there with interesting associated quantities that could be estimated using these techniques.
